...
(function($, undefined) {
	...
	adonisJPlayerPlaylist = function(cssSelector, playlist, options) {
		...
		}, this._options, options); // Object: The jPlayer constructor options
									// for this playlist and the playlist
									// options
		...
		// playlist.
		this.cssSelector.details = this.cssSelector.cssSelectorAncestor
				+ " .jp-details"; // Note that jPlayer controls the text in
									// the title element.
		...
		$(this.cssSelector.jPlayer).bind($.jPlayer.event.resize,
				function(event) {
		...
		$(this.cssSelector.shuffle).click(
				function(e) {
					e.preventDefault();
					if (self.shuffled
							&& $(self.cssSelector.jPlayer).jPlayer("option",
									"useStateClassSkin")) {
						self.shuffle(false);
					} else {
						self.shuffle(true);
		...
		// Create .on() handlers for the playlist items along with the free
	...
	adonisJPlayerPlaylist.prototype = {
		...
		_refresh : function(instant) {
			...
			if (instant && !$.isFunction(instant)) {
				...
				$.each(this.playlist, function(i) {
					$(self.cssSelector.playlist + " ul").append(
							self._createListItem(self.playlist[i]));
				});
				this._updateControls();
			} else {
				var displayTime = $(this.cssSelector.playlist + " ul")
						.children().length ? this.options.playlistOptions.displayTime
						: 0;
						...
						.slideUp(
								...
								function() {
									...
									$
											.each(
													...
													function(i) {
														$this
																.append(self
																		._createListItem(self.playlist[i]));
													});
									...
									if (self.playlist.length) {
										$(this)
												.slideDown(
														self.options.playlistOptions.displayTime);
									} else {
										$(this).show();
									}
								});
		...
		_createListItem : function(media) {
			...
			// if($.jPlayer.prototype.format[property]) { // Check property is a
			// media format.
			// if(first) {
			// first = false;
			// } else {
			...
			// }
			// listItem += "<a class='" +
			// self.options.playlistOptions.freeItemClass + "' href='" + value +
			...
			// }
			//
			// // The title is given next in the HTML otherwise the float:right
			// on the free media corrupts in IE6/7
			// listItem += "<a href='javascript:;' class='" +
			// this.options.playlistOptions.itemClass + "' tabindex='0'>" +
			// media.title + (media.artist ? " <span class='jp-artist'>by " +
			// media.artist + "</span>" : "") + "</a>";
			// listItem += "</div></li>";
			//
			// return listItem;
			...
			// // The title is given next in the HTML otherwise the float:right
			// on the free media corrupts in IE6/7
			// listItem += '<div class="img-box"><img
			// src="'+media.poster+'"></div>';
			// //listItem += '<div class="info">';
			// listItem += "<a href='javascript:;' class='" +
			// this.options.playlistOptions.itemClass + "' tabindex='0'>" +
			// media.title + "</a>";
			// listItem += "<p><a href='#'>"+(media.artist ? " <span
			// class='jp-artist'>by " + media.artist + "</span>" : "")+"</a></p>
			// ";
			// //listItem += '</div>';
			// listItem += "</div></li>";
			//
			...
			var re = /{(.*?\})/, strRe = typeof media.artist !== 'undefined' ? media.artist
					.replace(re, '')
					: null, Match = typeof media.artist !== 'undefined' ? media.artist
					.match(re, '')
					: null, artistHtml = '';
			...
			var listItem = "<li class='item clearfix' id='" + ID
					+ "'><div class='playlist-item'>";
			listItem += '<div class="img-box music-img-box song-poster"><img src="'
					+ media.poster
					+ '" alt=""><div class="hover-state"><span class="play-btn-dark"><i class="play-icon"></i></span></div></div>';
			...
			audio.onloadedmetadata = function() {
				var durationHtml = '';
				var minutes = Math.floor(Math.floor(audio.duration) / 60), seconds = Math
						.floor(audio.duration) % 60;
				durationHtml = '<span class="jp-time">' + minutes + ':'
						+ seconds + '</span>';
			...
			listItem += "<a href='javascript:;' class='"
					...
					+ media.title + "</a>";
			listItem += media.artist ? "<p class='jp-artist'>" + artistHtml
					+ "</p>" : "";
			// listItem += "</div>";
		...
		_createItemHandlers : function() {
			...
			$(this.cssSelector.playlist).off("click",
					"a." + this.options.playlistOptions.itemClass).on("click",
					"a." + this.options.playlistOptions.itemClass, function(e) {
						e.preventDefault();
			...
			$(this.cssSelector.playlist).off("click",
					...
					function(e) {
						...
						$(this).parent().parent().find(
								"." + self.options.playlistOptions.itemClass)
								.click();
						self.blur(this);
					});
		...
		_updateControls : function() {
			...
			if (this.shuffled) {
				$(this.cssSelector.jPlayer)
						.jPlayer("addStateClass", "shuffled");
			} else {
				$(this.cssSelector.jPlayer).jPlayer("removeStateClass",
						"shuffled");
			}
			if ($(this.cssSelector.shuffle).length
					&& $(this.cssSelector.shuffleOff).length) {
				if (this.shuffled) {
		...
		_highlight : function(index) {
			if (this.playlist.length && index !== undefined) {
				...
				$(
						this.cssSelector.playlist + " li:nth-child("
								+ (index + 1) + ")").addClass(
						"jp-playlist-current").find(".jp-playlist-item")
						.addClass("jp-playlist-current");
				// $(this.cssSelector.details + " li").html("<span
				// class='jp-title'>" + this.playlist[index].title + "</span>" +
		...
		add : function(media, playNow) {
			$(this.cssSelector.playlist + " ul").append(
					this._createListItem(media)).find("li:last-child").hide()
			...
			if (shuffled || shuffled !== this.shuffled) {
				...
				$(this.cssSelector.playlist + " ul").slideUp(
						...
						function() {
							...
							$(this).slideDown(
									self.options.playlistOptions.shuffleTime);
						});
			}
		},
		blur : function(that) {
